...
stormDataFinal <- stormData %>%
    ...
    select(event, EVTYPE, fatalities = FATALITIES, injuries = INJURIES, 
...
```{r Examining summary statistics on population health indicators}

stormDataFinal %>%
    group_by(event) %>%
    summarise(across(c('fatalities', 'injuries'), sum, .names = '{.col}_sum')) %>%
    arrange(desc(fatalities_sum), desc(injuries_sum))
```

Our second Research Question can be addressed in much the same way, but instead looking at summary statistics on the `` and `` variables, as they are the two indicators of economic impact that we created through an appropriate transformation of the original data set as described in the previous section:

```{r Examining summary statistics on economic consequences}

stormDataFinal %>%
    group_by(event) %>%
    summarise(across(c('propertydamage', 'cropdamage'), sum, .names = '{.col}_sum')) %>%
    arrange(desc(propertydamage_sum + cropdamage_sum))
```
